  ...
  function check_aco_dynamic_pricing_active() {
    ...
    function awcdp_calculated_total($cart_total, $cart) {
       ...
       function awcdp_build_payment_schedule($remaining_amounts, $deposit, $cart_items_deposit_amount){
        ...
        foreach ($schedule as $payment_key => $payment) {
            ...
            // Add a third payment with a fixed amount of 1000
    $schedule[$third] = array(
        'id' => '',
        'title' => esc_html__('Deposite for rental', 'deposits-partial-payments-for-woocommerce'),
        'type' => 'third_payment',
        'total' => 1000.0,
    );
    return $schedule;
    ...
    function awcdp_checkout_update_order_meta($order_id){
        ...
        if (isset(WC()->cart->deposit_info['deposit_enabled']) && WC()->cart->deposit_info['deposit_enabled'] === true) {
            ...
            $order->add_meta_data('_awcdp_deposits_third_payment_paid', 'no', true);
            ...
            $order->add_meta_data('_awcdp_deposits_third_payment', $third_payment, true);
            ...
            $order->add_meta_data('_awcdp_deposits_third_payment_reminder_email_sent', 'no', true);
            $order->save();
            ...
            if ($has_deposit == 'yes') {
                //TODO: update delete meta for third order
          ...
          $partial_editor = $screen->id === 'awcdp_payment' || $screen->id === 'edit-awcdp_payment' || $screen->id === 'woocommerce_page_wc-orders--awcdp_payment';
            ...
            if ($partial_editor) {
              ...
              if ($parent) {
                ...
                elseif ($order->get_meta('_awcdp_deposits_payment_type', true) == 'third_payment') {
                    $parent->update_meta_data('_awcdp_deposits_third_payment_paid', 'yes');
                  }
                $parent->save();
            ...
            if ($has_deposit == 'yes') {
              ...
              $order->update_meta_data('_awcdp_deposits_third_payment_paid', 'yes');
              $order->save();
            }
      }
    }
    ...
    function awcdp_early_update_partial_payments($order_id){
      ...
      if ($order) {
        ...
        if (is_array($schedule)){
          ...
          $order->update_meta_data('_awcdp_deposits_third_payment_paid', 'no');
          ...
          $order->update_meta_data('_awcdp_deposits_third_payment_reminder_email_sent', 'no');
      $order->save();
        }
      }
    }
    ...
    function awcdp_get_order_item_totals($total_rows, $order){
        ...
        if ($has_deposit){
          ...
          $deposit_amount_text = ( isset($awcdp_ts['deposit_amount_text']) && ( $awcdp_ts['deposit_amount_text'] != '' ) ) ? $awcdp_ts['deposit_amount_text'] : esc_html__('Deposit Amount', 'deposits-partial-payments-for-woocommerce' );
            ...
            $third_payment = floatval($order->get_meta('_awcdp_deposits_third_payment', true));
            $third_payment_paid = $order->get_meta('_awcdp_deposits_third_payment_paid', true);
            ...
            if ($is_checkout && !$is_remaining && !$is_email) {
                ...
                } elseif ($deposit_paid === 'yes' && $second_payment_paid === 'yes' && $third_payment_paid !== 'yes' ) {
                    $to_pay = $third_payment;
                  }
        ...
        $deposit_text = ( isset($awcdp_ts['deposit_amount_text']) && ( $awcdp_ts['deposit_amount_text'] != '' ) ) ? $awcdp_ts['deposit_amount_text'] : esc_html__('Deposit Amount', 'deposits-partial-payments-for-woocommerce' );
          ...
          $second_payment = floatval($order->get_meta('_awcdp_deposits_second_payment', true));
              ...
              $future_amount = wc_price($second_payment, array('currency' => $order->get_currency()));
                        $third_amount = wc_price($third_payment, array('currency' => $order->get_currency()));

              return '<br><span><small>('.esc_html($deposit_text).': '.$deposit_amount.'<br>'.esc_html($future_text).': '.$future_amount + $third_amount.')</small></span>';
